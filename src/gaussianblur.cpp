...
template<typename T> void getGaussian(T& kernel, const float sigma, int width = 0, int FFT_length = 0)
{
    ...
    const float sum = 1.0f / std::accumulate(kernel.begin(), kernel.begin() + width, 0.0f);

    std::transform(kernel.begin(), kernel.begin() + width, kernel.begin(), [&sum](auto& i) { return i * sum; });
...
void pffft_sorted_optimized_convolution(std::vector<T, N>& tile_dft, const std::vector<T, N>& kernel_dft, float scaler) {
    ...
    for (int i = 0; i < tile_dft.size() / 2; i++) {
        ...
        const float real_part_kernel_multiplier = kernel_dft.at(real_part_idx) * scaler;
...
KernelDFT prepare_kernel_DFT(const ImgGeom image_geometry, const float nsmooth) {
	...
	// absolute min padd
	std::array<int, 2> sizes = { image_geometry.rows + pad * 2, image_geometry.cols + pad * 2 };
	...
	for (int i = 0; i < 2; ++i) {
		if (!isValidSize(sizes.at(i)))
		{
			int new_size = nearestTransformSize(sizes.at(i));
    ...
    getGaussian(kernel_aligned_1D_col, sigma, kSize, sizes.at(1));
	...
	pffft_transform_ordered(cols_setup.get(), kernel_aligned_1D_col.data(), kerf_1D_col.data(), tmp.data(), PFFFT_FORWARD);
	...
	if (sizes.at(0) != sizes.at(1)) {
        ...
        getGaussian(kernel_aligned_1D_row, sigma, kSize, sizes.at(0));

		tmp.resize(sizes.at(0));
		pffft_transform_ordered(rows_setup.get(), kernel_aligned_1D_row.data(), kerf_1D_row.data(), tmp.data(), PFFFT_FORWARD);
...
std::optional<DeinterleavedChs> prepare_work_array(Image& image) {
	...
	if (image.geom.channels == 3) {
		deinterleaved_vector = DeinterleavedChs(3, std::vector<float>(img_size_per_channel));
		std::array<float*, 3> BGR = { deinterleaved_vector.at(0).data(), deinterleaved_vector.at(1).data(), deinterleaved_vector.at(2).data() };
	    deinterleave_channels<3>(image.data.data(), BGR.data(), image.geom.rows * image.geom.cols);
	}
	else if (image.geom.channels == 4) {
		deinterleaved_vector = DeinterleavedChs(4, std::vector<float>(img_size_per_channel));
		std::array<float*, 4> BGRA = { deinterleaved_vector.at(0).data(), deinterleaved_vector.at(1).data(), deinterleaved_vector.at(2).data(), deinterleaved_vector.at(3).data() };
		deinterleave_channels<4>(image.data.data(), BGRA.data(), image.geom.rows * image.geom.cols);
	}
	else {
...
void process_channel_tiles(
	...
	const int tile_size,
	const int pad,
	const int trailing_zeros,
	PFFFT_Setup* setup,
	const AlignedVector<float> &kernel,
	AlignedVector<float> &tmp,
	...
	float scaler) {
	    ...
	    hybrid_loop(tiles, [&](auto j){
			...
			pffft_transform_ordered(setup, work_local.data(), tile_local.data(), tmp_local.data(), PFFFT_BACKWARD);

			// save the 1st pass tile per tile in the output vector
			std::copy_n(tile_local.begin() + pad, tile_size, resf.begin() + j * tile_size);
		});
...
void pffft(const ImgGeom image_geometry, const KernelDFT kernelDFT, DeinterleavedChs& temp, bool alpha)
{
	...
	for (int i = 0; i < ch_to_process ; ++i) {
		...
		process_channel_tiles(
			...
			image_geometry.cols,
			kernelDFT.pad,
			kernelDFT.trailing_zeros.cols,
			kernelDFT.cols_setup.get(),
			kernelDFT.kerf_1D_col,
			tmp,
			tile,
			work,
			resf,
