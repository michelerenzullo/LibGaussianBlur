...
void deinterleave_channels(const T* const interleaved, U** const deinterleaved, const uint32_t total_size) {

    // Cache-friendly deinterleave, splitting for blocks of 16 MB, inspired by flip-block
    constexpr float round = std::is_integral_v<U> ? std::is_integral_v<T> ? 0 : 0.5f : 0;
    constexpr uint32_t block = L2_CACHE_SIZE / (Channels * std::max(sizeof(T), sizeof(U)));
...
template<uint32_t Channels, typename T, typename U>
void interleave_channels(const U** const deinterleaved, T* const interleaved, const uint32_t total_size) {
