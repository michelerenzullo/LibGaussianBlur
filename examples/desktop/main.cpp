...
void write_image(std::string file, const Image& image)
{
	...
	if (ext == "bmp")
		stbi_write_bmp(file.c_str(), image.geom.cols, image.geom.rows, image.geom.channels, image.data.data());
	else if (ext == "jpg")
		stbi_write_jpg(file.c_str(), image.geom.cols, image.geom.rows, image.geom.channels, image.data.data(), 100);
	else
	{
		if (ext != "png")
		{
		...
		stbi_write_png(file.c_str(), image.geom.cols, image.geom.rows, image.geom.channels, image.data.data(), image.geom.channels * image.geom.cols);
	}
}

std::optional<std::vector<uint8_t>> read_image(const std::string& file, int& cols, int& rows, int& channels)
{
    std::unique_ptr<uint8_t, STBImageDeleter> image_data(stbi_load(file.c_str(), &cols, &rows, &channels, 0));
    if (!image_data) {
