...
std::vector<unsigned char> write_image_to_memory(const Image& image, const std::string& ext)
{
    ...
    if (ext == "jpg" || ext == "jpeg") {
        stbi_write_jpg_to_func(
            ...
            &memory_buffer, image.geom.cols, image.geom.rows, image.geom.channels, image.data.data(), 100);
    } else {
        ...
        stbi_write_png_to_func(
            ...
            &memory_buffer, image.geom.cols, image.geom.rows, image.geom.channels, image.data.data(), image.geom.channels * image.geom.cols);
    }

    return memory_buffer;
}

std::optional<std::vector<uint8_t>> read_image(const std::unique_ptr<uint8_t[], free_delete> file, const size_t file_size, int& cols, int& rows, int& channels)
{
    std::unique_ptr<uint8_t, STBImageDeleter> image_data(stbi_load_from_memory(file.get(), file_size, &cols, &rows, &channels, 0));
    if (!image_data) {
    ...
    if (channels != 3 && channels != 4) {
        throw std::runtime_error("Image format not supported, only RGB and RGBA images are supported");
    }
...
em_val run(uintptr_t args_ptr, uintptr_t target_ptr, size_t target_size)
{
  ...
  args.append(delimiter); // add final "-" in order to read the last argument inside the for loop
  size_t pos = 0;
  args.erase(0, args.find(delimiter) + strlen(delimiter)); // ignore the first empty entry "" before the first "-"
  for (std::string arg; (pos = args.find(delimiter)) != std::string::npos; args.erase(0, pos + strlen(delimiter)))
  {
    ...
    if (sigma > 0)
	{
	    ...
	    if ((image_data = read_image(std::move(target), target_size, cols, rows, channels)).has_value()) {
          ...
          file_copy = Uint8Array.new_(emscripten::typed_memory_view(img_in_mem.size(), img_in_mem.data()));
        }
  	}
