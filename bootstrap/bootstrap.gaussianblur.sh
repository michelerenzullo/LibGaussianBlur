...
compile_gaussian_blur() {
    ...
    if [ "$PLATFORM" = "android" ] ; then
        ...
        if [ -d "${FINAL_PREFIX_DIR}/include/$PREFIX" ] ; then rm -rf "${FINAL_PREFIX_DIR}/include/$PREFIX" ; fi
        mv ${TEMP_PREFIX_DIR}/include/gaussianblur ${FINAL_PREFIX_DIR}/include/
        mv ${TEMP_PREFIX_DIR}/lib/*.a* ${FINAL_PREFIX_DIR}/lib/
        mv ${TEMP_PREFIX_DIR}/bin/* ${FINAL_PREFIX_DIR}/bin/
        cp $(git_root)/examples/wasm/multi-thread/gaussianblur.html ${FINAL_PREFIX_DIR}/bin/
        ...
        cp $(git_root)/examples/wasm/multi-thread/gaussianblur_worker.js ${FINAL_PREFIX_DIR}/bin/
        sed -i '' "s|const Module = h|const Module = "${MODULE_NAME}";|" "${FINAL_PREFIX_DIR}/bin/gaussianblur_worker.js"
